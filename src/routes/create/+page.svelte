<script>
  ...
  <form class="space-y-2" method="POST" action="?/create">
      
  <Label class="space-y-2">
    <span>Title</span>
    <Input type="text" name="title" value={form?.title ?? ''} placeholder="Default input" size="md" />
  </Label>
    <label for="editor" class="sr-only">Publish post</label>
    <Textarea id="editor" rows="8" class="mb-4" value={form?.content ?? ''} placeholder="Write a comment" name="content">
